...
class Nodelet(Node):
    def __init__(self):
        ...
        #odom param
        self.wheel_separation = 0.205  # Adjust as necessary
    ...
    def timer_callback(self):
        ...
        if JOY_CONTROL:
            vel_input1 =  self.v_gain*self.joy_fb
            vel_input1 += self.w_gain*self.joy_lr
            vel_input2 =  self.v_gain*self.joy_fb
            vel_input2 -= self.w_gain*self.joy_lr

            self.md.send_vel_cmd(vel_input1, vel_input2)
        ...
        self.msg_wheelmotor.velocity1 = int(self.md.gear * self.md.rpm1)
        self.msg_wheelmotor.velocity2 = int(self.md.gear * self.md.rpm2)
        self.msg_wheelmotor.current1 = self.md.current1
        self.msg_wheelmotor.current1 = self.md.current2
        self.msg_wheelmotor.target1 = int(vel_input1)
        self.msg_wheelmotor.target2 = int(vel_input2)
        self.pub.publish(self.msg_wheelmotor)
        
#############################odom 5/30################################
        # RPM to m/s
        left_wheel_speed = (self.md.gear * self.md.rpm1 / 60.0) * (np.pi * self.wheel_diameter)
        right_wheel_speed = (self.md.gear * self.md.rpm2 / 60.0) * (np.pi * self.wheel_diameter)
        linear_velocity = (right_wheel_speed + left_wheel_speed) / 2.0
        angular_velocity = (right_wheel_speed - left_wheel_speed) / self.wheel_separation
