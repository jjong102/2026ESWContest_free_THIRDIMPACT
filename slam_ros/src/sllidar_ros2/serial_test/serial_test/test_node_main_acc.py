...
class Nodelet(Node):
    def __init__(self):
        ...
        self.gain_count = 2
        self.gain_list = [0.5, 0.7, 1.0, 1.3, 1.5, 1.7, 2.0, 2.3, 2.5, 7.0]
        ...
        self.max_delta = 70.0  # 1 loop당 최대 속도 변화량 (rpm)
    ...
    def timer_callback(self):
        ...
        if self.JOY_CONTROL:
            ...
            raw_vel1 = self.v_gain * self.joy_fb - self.w_gain * self.joy_lr
            raw_vel2 = self.v_gain * self.joy_fb + self.w_gain * self.joy_lr
            

            self.vel_input1 = self.smooth_velocity_transition(self.vel_input1, raw_vel1, self.max_delta)
            self.vel_input2 = self.smooth_velocity_transition(self.vel_input2, raw_vel2, self.max_delta)
    ...
    def smooth_velocity_transition(self, current, target, max_delta):
    ...
    def cmd_vel_callback(self, msg):
        if not self.JOY_CONTROL:
            ...
            encoder_delta_right = self.smooth_velocity_transition(
                self.cmd_vel_r_old, encoder_delta_right, self.max_delta
            )
            encoder_delta_left = self.smooth_velocity_transition(
                self.cmd_vel_l_old, encoder_delta_left, self.max_delta
            )
            self.cmd_vel_r_old = encoder_delta_right
            self.cmd_vel_l_old = encoder_delta_left
