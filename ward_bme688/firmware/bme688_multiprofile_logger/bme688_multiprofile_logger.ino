/**
 * bme688_multiprofile_logger.ino
 *
 ...
 *
 * [10] 히터 프로파일 4종 병행 (Bosch 원본 탐색 모드 복원)
 *     문제: 8개 센서를 모두 같은 온도 프로그램으로 돌리면
 *           같은 측정을 8번 반복하는 것과 같아 정보가 10차원에 머문다.
 *           실제로 특징 공학을 해봐도 성능이 오르지 않았다(0.526 → 0.516).
 *     수정: Bosch 원본 설정처럼 4가지 프로파일을 2센서씩 배정한다.
 *           서로 다른 온도 프로그램은 서로 다른 화학 정보를 끌어내므로
 *           파서에서 합치면 40차원 특징이 된다.
 *
 *       센서 0,1 → HP354  표준     : 320 초기화 후 100C 유지시간 2/10/30 (11.5초)
 *       센서 2,3 → HP501  완만     : 210-265-320-265-210-155-100-155 (26.9초)
 *       센서 4,5 → HP411  급전이   : 100↔320 왕복 후 240 유지 (24.6초)
 *       센서 6,7 → HP301  장시간   : 100/200/320 각각 길게 유지 (18.3초)
 *
 *     ※ 이 펌웨어로 찍은 데이터는 parse_devkit_multi.py 로 파싱해야 한다.
 *       기존 단일 프로파일 데이터와는 호환되지 않는다(전면 재수집 필요).
 *
 * === 조작 요약 ===
 *   SW_START | 'p' : 시작 / 일시정지
...
#define AUTO_BLOCK_MS   420000UL  /* 자동 모드일 때의 블록 길이 (5분)
                                   * [10] 느린 프로파일이 27초라 3분이면 표본이 너무 적다 */

#define START_DELAY_MS  5000UL   /* 시작 전 대기 (손 떼고 물러날 시간) */

/* ---- [10] 히터 프로파일 4종 (Bosch 원본 .bmerawdata 에서 그대로 가져옴) ----
 * 값을 바꾸면 학습 데이터와 호환이 깨진다. 파서의 표와도 반드시 일치해야 한다. */
#define N_PROFILE 4

const uint16_t PROF_TEMP[N_PROFILE][10] = {
...
const uint16_t PROF_MUL[N_PROFILE][10] = {
...
const char* PROF_NAME[N_PROFILE] = {"HP354", "HP501", "HP411", "HP301"};

/* 센서 → 프로파일. 2개씩 묶어 같은 프로파일을 준다.
 * 파서는 {0,2,4,6} 을 배열 A, {1,3,5,7} 을 배열 B 로 보고
 * 각각 4프로파일 x 10스텝 = 40차원을 만든다. */
static inline uint8_t profileOf(uint8_t sensor) { return sensor / 2; }
...
void setup(void)
{
  ...
  /* --- [10] 센서마다 다른 히터 프로파일을 배정한다 ---
   * 이 표를 바꾸면 학습 데이터와 호환이 깨진다. 파서 표와도 일치해야 한다. */
  Serial.println();
  Serial.println(F("히터 프로파일 배정:"));

  for (uint8_t i = 0; i < N_KIT_SENS; i++) {
    bme[i].setTPH();

    uint8_t  p = profileOf(i);
    uint16_t tempProf[10], mulProf[10];
    for (uint8_t s = 0; s < 10; s++) {
      tempProf[s] = PROF_TEMP[p][s];
      mulProf[s]  = PROF_MUL[p][s];
    }
    ...
    /* 프로파일마다 한 번씩만 사이클 길이를 알려준다 */
    if (i % 2 == 0) {
      /* 한 하위 측정에 걸리는 시간은 '가열(sharedHeatrDur) + TPH 측정' 이고
       * 그 합이 곧 MEAS_DUR 이다. sharedHeatrDur 만 곱하면 약 70% 로 과소평가된다. */
      uint32_t cycleMs = 0;
      for (uint8_t s = 0; s < 10; s++) cycleMs += MEAS_DUR * mulProf[s];
      Serial.print(F("  센서 "));  Serial.print(i);
      Serial.print(F(","));        Serial.print(i + 1);
      Serial.print(F(" → "));      Serial.print(PROF_NAME[p]);
      Serial.print(F("   사이클 약 "));
      Serial.print(cycleMs / 1000.0, 1);
      Serial.println(F(" 초"));
    }
  }
  Serial.println(F("※ 가장 느린 프로파일이 약 27초이므로 블록을 5분 이상 권장"));
  Serial.println(F("   5분 블록이면 클래스당 약 47개 샘플이 나온다"));
  ...
  Serial.println(F("파싱: parse_devkit_multi.py (40차원). 기존 파서와 호환 안 됨"));
