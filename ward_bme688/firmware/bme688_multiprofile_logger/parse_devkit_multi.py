...
나머지 세 프로파일은 '그 시점 이전에 끝난 가장 최근 사이클' 을 가져다 쓴다.
  ...
  - 결과적으로 약 11초마다 한 샘플이 나온다 (5분 블록에 배열당 약 24개, 두 배열 합쳐 약 47개)
...
# 사이클 길이 = MEAS_DUR(140ms) x 배수 합.
# 한 하위 측정에 걸리는 시간은 '가열(sharedHeatrDur) + TPH 측정' 이며
# 그 합이 곧 MEAS_DUR 이다. 가열분만 곱하면 약 70% 로 과소평가된다.
# 실측 확인: 5분 블록에서 HP354 가 26사이클 → 300/26 = 11.5초
...
MAX_AGE_S = 40.0            # 다른 프로파일 사이클이 이보다 오래되면 버림.
                            # 가장 느린 HP501 이 26.9초이고, 중간에 사이클이 폐기되면
                            # 지연이 더 늘어난다(실측 최대 38초). 40초면 대부분 살릴 수 있다.
