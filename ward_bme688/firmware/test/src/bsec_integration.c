...
static bsec_library_return_t bme68x_bsec_update_subscription(float sample_rate, uint8_t sens_no)
{
    ...
    status = bsec_update_subscription(bsecInstance[sens_no], requested_virtual_sensors, n_requested_virtual_sensors, required_sensor_settings,
                    ...
                    bme68x_read_fptr_t bus_read, sleep_fct sleep, state_load_fct state_load, config_load_fct config_load,  struct bme68x_dev dev, uint8_t sens_no)
{
    ...
    {       
        ret.bsec_status = bsec_set_configuration(bsecInstance[sens_no], bsec_config, bsec_config_len, work_buffer, sizeof(work_buffer));
        if (ret.bsec_status != BSEC_OK)
        {
            return ret;
        }
...
static bsec_library_return_t bme68x_bsec_process_data(bsec_input_t *bsec_inputs, uint8_t num_bsec_inputs, output_ready_fct output_ready, uint8_t sens_no)
{
    ...
    if (num_bsec_inputs > 0)
    {
        ...
        bsec_status = bsec_do_steps(bsecInstance[sens_no], bsec_inputs, num_bsec_inputs, bsec_outputs, &num_bsec_outputs);
    ...
    while (1)
    {
		...
		for (uint8_t sens_no = 0; sens_no < NUM_OF_SENS; sens_no++) { 
			...
			if (time_stamp >= sensor_settings[sens_no].next_call)
			{
				...
				bsec_status = bsec_sensor_control(bsecInstance[sens_no], time_stamp, &sensor_settings[sens_no]);
				if (bsec_status != BSEC_OK)
				{
					if (bsec_status < BSEC_OK)
					{
				...
				{
					bsec_status = bsec_get_state(bsecInstance[sens_no], 0, bsec_state, sizeof(bsec_state), work_buffer, sizeof(work_buffer), &bsec_state_len);
					if (bsec_status == BSEC_OK)
					{
